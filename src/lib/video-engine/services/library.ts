...
import { DRIVE_REUSABLE_CLIPS_FOLDER, ensureDriveWorkspace } from "./drive-workspace";
...
async function listRunFoldersUnderChannel(opts: {
  drive: DriveClient;
  channelFolderId: string;
  channelName: string;
}): Promise<LibraryRunSummary[]> {
  const reusable = await opts.drive.files.list({
    q: `'${opts.channelFolderId}' in parents and name='${DRIVE_REUSABLE_CLIPS_FOLDER.replace(/'/g, "\\'")}' and mimeType='application/vnd.google-apps.folder' and trashed=false`,
    fields: "files(id, name)",
    pageSize: 1,
  });
  const reusableFolderId = reusable.data.files?.[0]?.id;
  if (!reusableFolderId) return [];

  const runFolders = await opts.drive.files.list({
    q: `'${reusableFolderId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`,
  ...
  const runResults = await Promise.all(
    (runFolders.data.files ?? []).map((rf) =>
      rf.id && rf.name
        ? readRunFolder(opts.drive, { id: rf.id, name: rf.name, createdTime: rf.createdTime }, opts.channelName)
        : Promise.resolve(null)
    )
  );
  return runResults.filter((r): r is LibraryRunSummary => !!r);
}

async function listModernLibraryRuns(
  drive: DriveClient,
  channelFilter?: string
): Promise<LibraryRunSummary[]> {
  const { channelsFolderId } = await ensureDriveWorkspace();
  const channelFolders = await drive.files.list({
    q: `'${channelsFolderId}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false`,
    fields: "files(id, name)",
    pageSize: 500,
  });
  ...
  for (const ch of channelFolders.data.files ?? []) {
    ...
    summaries.push(
      ...(await listRunFoldersUnderChannel({
        drive,
        channelFolderId: ch.id,
        channelName,
      }))
    );
  }
...
async function listLegacyLibraryRuns(
  drive: DriveClient,
  channelFilter?: string
): Promise<LibraryRunSummary[]> {
  ...
  for (const ch of channelFolders.data.files ?? []) {
    ...
    for (const r of runResults) {
      if (r && (!channelFilter || r.channel === channelFilter)) summaries.push(r);
    }
  }
  return summaries;
}
...
/**
 * Lists every reusable generated run in Drive, newest first.
 *
 * New layout: Channels / {channel} / 02 Reusable Scene Clips / {run}
 * Legacy fallback: Clips Library / {channel} / {run}
 */
export async function listLibraryRuns(options: { channel?: string } = {}): Promise<LibraryRunSummary[]> {
  const drive = getDriveClient();
  if (!drive) return [];

  const channelFilter = options.channel?.trim();
  const modern = await listModernLibraryRuns(drive, channelFilter).catch(() => []);
  const legacy = await listLegacyLibraryRuns(drive, channelFilter).catch(() => []);
  const summaries: LibraryRunSummary[] = [];
  const seen = new Set<string>();
  for (const run of [...modern, ...legacy]) {
    const key = run.run_id || run.drive_folder_id;
